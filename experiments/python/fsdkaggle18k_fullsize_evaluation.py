...
class FSDKaggle18KFullsizeEvalution():
    ...
    def forward(self, verbose: bool = False):
        ...
        history, classwise_res = dict(zip(_labelset, [[] for _ in _labelset])), dict()
        if self.fewshot_cfgs['n_supports'] == 0:
            ...
            cls_results, cls_counts = self._zero_shot_classification(model=clap_model, eval_dataloader=self.eval_dataloader, a=self.a, b=self.b, **self.fewshot_cfgs)
            ...
            for xs, ys in self.train_dataloader:
                if self.fine_tune:
                    cls_results, cls_counts = self._adapt_on_batch(model=clap_model, eval_dataloader=self.eval_dataloader, wav_paths=xs, targets=ys, a=self.a, b=self.b, train_a=self.train_a, **self.fewshot_cfgs, **self.tr_cfgs)
                else:
                    cls_results, cls_counts = self._test_on_batch(model=clap_model, eval_dataloader=self.eval_dataloader, wav_paths=xs, targets=ys, a=self.a, b=self.b, **self.fewshot_cfgs)
                for cat, cnt in cls_results.items():
                    acc = cnt / cls_counts[cat]
                    if verbose:
                        print(f"Class {cat} running class-wise accuracy: {acc}")
                    history[cat].append(acc)
        for cat, acc in history.items():
            classwise_res[cat] = (torch.tensor(acc, dtype=torch.float).mean()).detach().item()
        return classwise_res, history

    def _adapt_on_batch(self, model: torch.nn.Module, eval_dataloader: torch.nn.Module, wav_paths: list, targets: list, n_class: int, n_supports: int, n_queries: int, a: float, b: float, train_a: bool, train_epochs: int, train_lr: float) -> Tensor:
